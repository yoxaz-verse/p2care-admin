...
import generateData, { generateLeadsType } from '@/app/content/tableData'
...
const Content = () => {
  ...
  const { data: getDesignation, isLoading: isLoadingDesignation, isError: isErrorDesignation, isSuccess: isSuccessDesignation } = useQuery({
  ...
  const { data: getGender, isLoading: isLoadingGender, isError: isErrorGender, isSuccess: isSuccessGender } = useQuery({
  ...
  const { isOpen: isOpenUpdate, onOpen: onOpenUpdate, onOpenChange: onOpenChangeUpdate, onClose: onCloseView } = useDisclosure();
  ...
  return (
    <div className='flex flex-col w-full'>
      ...
      <div className='flex flex-row justify-between items-center w-full self-end p-[1rem]'>
        ...
        <AddModal isOpen={isOpen} onOpen={onOpen} onOpenChange={onOpenChange} title={"Gender"}>
          <form className='flex flex-col items-center gap-4 justify-center' onSubmit={(e) => handleSubmit(e)}>
            <Input
              value={gender}
              onValueChange={(e) => setGender(e)}
              type='text'
              placeholder='Write the gender you want to add' />
            <Button isLoading={loading} type="submit" className='w-full' color="primary">
              Create
            </Button>
          </form>
        </AddModal>
      ...
      {
        generateTable({
          columns: generateTabColumns({ onView: (type: any) => handleView(type), setData: setData, tableName: "Gender" }),
      ...
      <UpdateModal isOpen={isOpenUpdate} onOpenChange={onOpenChangeUpdate} title='Gender'>
        <form onSubmit={(e) => handleUpdateSubmit((e))} className='flex flex-col items-center w-full gap-4 justify-around'>
          <Input value={data?.name} placeholder='Update the name'
            onValueChange={(e) => setData((data: any) => ({
              ...data,
              name: e
            }))} />
          <Button isLoading={loading} type='submit' color='primary' radius='full' className='w-full'>Submit</Button>
        </form>
      </UpdateModal>
      <DeleteModal isOpen={isOpenDelete} onOpenChange={onOpenChangeDelete} title='Gender' api={`${GenderRoutes.gender}/${data}`} queryKey={"get-gender"} />
      <ViewModal data={data} size='sm' isOpen={isOpenView} onOpenChange={onOpenChangeView} title='Gender' keys={generateLeadsType("Gender")} />
      <div className='flex flex-row justify-between items-center w-full self-end p-[1rem]'>
        <h3 className='text-[20px] p-[1rem] font-bold'>Designation</h3>
        <AddModal isOpen={isOpen} onOpen={onOpen} onOpenChange={onOpenChange} title={"Designation"}>
          <form className='flex flex-col items-center gap-4 justify-center' onSubmit={(e) => handleSubmit(e)}>
            <Input
              value={gender}
              onValueChange={(e) => setGender(e)}
      ...
      {
        generateTable({
          columns: generateTabColumns({ onView: (type: any) => handleView(type), setData: setData, tableName: "Designation" }),
