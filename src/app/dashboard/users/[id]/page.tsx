...
import { BreadcrumbItem, Image, Input, Breadcrumbs, Button, Card, CardBody, CardHeader, useDisclosure, Avatar, Spinner } from "@nextui-org/react"
...
export default function UserName() {
  ...
  const { data: getPateintData, isLoading, isFetched } = useQuery({
  ...
  return (
    <div className="flex flex-col w-full">
      <div className="flex flex-col w-full">
        ...
        <DeleteModal data={id} isOpen={isOpen} onOpenChange={onOpenChange} title="User" api={patientRoutes.patient}
          queryKey={["user"]} />
      </div>
      <Card className="w-full" radius="lg" shadow="lg">
        ...
        <CardBody className="flex flex-col  gap-4   h-full">
          <form onSubmit={(e) => handleSubmit(e)} className="w-full flex flex-col h-full gap-4 items-center">
      ...
      <Page needAddModal={false} api={Doctor.appointments} apiKey="appointments" columns={appointmentColumns} title={`${getPateintData?.data.data.name}'s Appointment`} />
      <Page needAddModal={false} api={Doctor.enquiry} apiKey="enquiries" columns={enquiryColumns} title={`${getPateintData?.data.data.name}'s Enquiries`} />
    </div >
  )
}
